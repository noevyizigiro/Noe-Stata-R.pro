...
use "$Data/essay_complexity.dta" 
rename responseid id
merge 1:1 id using "$Data/Vyizigiro_Assignment1_CleanData.dta"
exit
drop _merge

preserve
import excel "$Data/Essay Scoring.xlsx", clear firstrow
rename x id
keep id score* Totalscore
drop if id==""
tempfile temp
save `temp'
restore
merge 1:1 id using `temp', assert(1 3) keep(3) nogen
...
//balancetable treated_AI age female other_gender white black asian hispanic other lower_income mid_income higher_income ai_familiarity regular_ai_use using "$pathOutput/balance_table1.xlsx", replace
...
*** Research question and graph

//the research question is abount understanding the effect of generative AI on learning

#delimit;
graph box score_total, over(treated_AI)  // using box
title("Total Score Across Two Groups") 
 ytitle("Score(0-10)") 
...
regress score_total treated_AI age race gender income, robust
