...
/*
IMF de facto classificaion: rogoff 2003

Pegged: Hard peggs, conventional peg
Floating: Independent floats, managed floats
Intermediate  Regime: the rest.
...
//categorizing the classificaitions within the similar categories (putting those with slightly similar name under the same category name)
...
egen Fixed= rowmax(curr_board peg_conv no_seplt)
egen Managed= rowmax(peg_crawl crawlike stabilized omanaged peg_hband manfloating)
egen Floating= rowmax(floating ffloating)


**label variables the grouped variables

local variables " Fixed Managed Floating"
local mylabels `" "Currency board (arrangement); (Exchange arrangement with) no separate legal tender; Conventional peg(arrangement)" "Crawl-like arrangement; Crawling peg; Pegged exchange rate within horizontal bands; Stabilized arrangement; Other managed arrangement" "(Free,independently) floating"  "'
